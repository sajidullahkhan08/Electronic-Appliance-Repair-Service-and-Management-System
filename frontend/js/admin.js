  ...
  loginForm.addEventListener('submit', async e => {
    ...
    try {
      ...
      if (json.success) {
        ...
        if (errEl) { errEl.textContent = json.error || 'Invalid credentials.'; errEl.style.display = 'block'; }
      }
    } catch {
      if (errEl) { errEl.textContent = 'Server unreachable.'; errEl.style.display = 'block'; }
    } finally {
      btn.disabled = false;
...
async function loadRequests(filter) {
  const url  = filter ? `${API}/requests?service_type=${encodeURIComponent(filter)}` : `${API}/requests`;
