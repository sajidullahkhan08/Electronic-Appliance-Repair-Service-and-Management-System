  ...
  radio.addEventListener("change", () => {
    if (addressRow) {
      addressRow.style.display =
        radio.value === "Home Service" ? "flex" : "none";
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    if (!data.phone) {
      showErr("err-phone", "Phone is required.");
      valid = false;
    }
    if (!data.appliance_type) {
      showErr("err-appliance", "Select an appliance type.");
      valid = false;
    }
    if (!data.problem_description) {
      showErr("err-problem", "Describe the problem.");
      valid = false;
    }
    if (!data.service_type) {
      showErr("err-service", "Choose a service type.");
      valid = false;
    }
    if (data.service_type === "Home Service" && !data.address) {
      showErr("err-address", "Address is required for Home Service.");
      valid = false;
    }
    if (!valid) return;
    ...
    try {
      const res = await fetch(
        window.apiUrl ? window.apiUrl("/request") : "/api/request",
        {
          method: "POST",
