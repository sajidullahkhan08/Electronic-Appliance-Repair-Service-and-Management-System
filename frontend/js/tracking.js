...
const SHOP_STATUSES = ['Pending', 'Under Inspection', 'Repairing', 'Completed', 'Ready for Pickup'];
const HOME_STATUSES = ['Pending', 'Scheduled', 'Technician Dispatched', 'Repairing', 'Completed'];
...
if (contactForm) {
  contactForm.addEventListener('submit', async e => {
    ...
    showLoading();
    try {
      const res  = await fetch(`/api/track-by-contact?query=${encodeURIComponent(q)}`, { credentials: 'include' });
...
async function doTrack(id) {
  ...
  resultBox.innerHTML = '';
  try {
    const res  = await fetch(`/api/track/${encodeURIComponent(id)}`, { credentials: 'include' });
    const json = await res.json();
    hideLoading();
    if (!json.success) {
      resultBox.innerHTML = notFoundCard(json.error || 'No record found for this Tracking ID.');
      return;
    }
...
function showLoading() { if (loadingBox) loadingBox.style.display = 'block'; if (resultBox) resultBox.innerHTML = ''; }
function hideLoading() { if (loadingBox) loadingBox.style.display = 'none'; }
function notFoundCard(msg) { return `<div class="card" style="text-align:center;padding:40px;"><div style="font-size:2.5rem;margin-bottom:16px;">❌</div><h3 style="margin-bottom:8px;">Not Found</h3><p>${msg}</p></div>`; }
function serverErrorCard() { return `<div class="card" style="text-align:center;padding:40px;"><p>Could not connect to server. Please try again.</p></div>`; }
